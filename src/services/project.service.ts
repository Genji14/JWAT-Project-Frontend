...
class ProjectService {
    ...
    addDocument = (projectId: number, form: FormData) => {
        return API_INSTANCE.post(PROJECT_ENDPOINTS.ADD_DOCUMENT(projectId), form, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
    ...
    getRootDocument = (projectId: number): Promise<AxiosResponse<IProjectRootDocument>> => {
        return API_INSTANCE.get(PROJECT_ENDPOINTS.GET_ROOT_DOCUMENT_GROUP(projectId));
    }
    ...
    deleteGroup = (groupId: number) => {
        return API_INSTANCE.delete(PROJECT_ENDPOINTS.DELETE_DOCUMENT_GROUP(groupId));
    }
