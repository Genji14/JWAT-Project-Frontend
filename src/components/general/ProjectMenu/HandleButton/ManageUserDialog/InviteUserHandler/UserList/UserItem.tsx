    ...
    return (
        <div className='w-full flex gap-2 items-center'>
            ...
            <div className='flex-auto'>
                <h4 className="font-semibold">{data.fullName} <span className="text-muted-foreground">#{data.id}</span></h4>
                <div className='text-xs text-muted-foreground flex items-end gap-1'>
                    <Mail className="w-3.5 h-3.5" />
                    <span>{data.email}</span>
                </div>
            </div>
            <Button disabled={isPendingInviteUser} onClick={() => handleInviteUser(data.id)} className='px-3 py-1.5'>
                Invite
            </Button>
        </div>
    )
