...
export const useGetBlogListSearch = (search: string | string[] | undefined) => {
    const {
        data,
        status,
        isFetching,
        ...
        queryKey: [BLOG_QUERY_KEY.GET_BLOG_LIST, search],
        queryFn: async ({ pageParam = 1 }) => {
            const res = await blogService.getBlogListSearch(pageParam, search!)
            return res.data
        },
        initialPageParam: 1,
        getNextPageParam: (lastPage) => {
